...
public class AuthorServiceTD5 {
	...
	public AuthorServiceTD5(EntityManager em) {
		super();
		this.em = em;
	}
	...
	public List<Author> listByLastName(String lastName) {
		log("listByLastName()");
		String requestPart1 = "SELECT a FROM Author a WHERE lastName " ;
		String pattern  = lastName ;
		String jpqlQuery = null ; // "SELECT a FROM Author a WHERE lastName = :name " ; // Nom de classe Java
		if ( lastName.endsWith("*")) {
			jpqlQuery = requestPart1 + " LIKE :name" ;
			pattern = lastName.substring(0, lastName.length()-1) + "%" ;
		}
		else {
			jpqlQuery = requestPart1 + " = :name" ;
			//jpqlQuery = "SELECT a FROM Author a WHERE lastName LIKE :name " ; // Nom de classe Java
		}
